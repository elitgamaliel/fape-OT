...
@Slf4j
public class InkaTrackerFirebaseService extends FirebaseBaseService implements FirebaseService {

    @Qualifier("inkaTrackerDatabase")
    @Autowired
    private DatabaseReference databaseReference;

    @Qualifier("inkaTrackerFirebaseAuthToken")
    @Autowired
    ...
    public void save(String path, FirebaseModel model) {
        log.info(">>IK New object: {} in path: {}", model.getId(), path);
    ...
    public void save(String path, List<FirebaseModel> models) {
    	log.info(">>IK New objects: {} in path: {}", models.stream().map(FirebaseModel::getId).collect(Collectors.toList()), path);
        DatabaseReference childReference = databaseReference.child(path);
    ...
    public void update(String path, FirebaseModel... models) {
    	log.info(">>IK Path: {} updated with: {}", path, Arrays.asList(models));
        DatabaseReference childReference = databaseReference.child(path);
    ...
    public void delete(String path, String id) {
    	log.info(">>IK Deleted id: {} from path: {}", id, path);
        DatabaseReference childReference = databaseReference.child(path);
        Map<String, Object> objectToUpdated = new HashMap<>();
    ...
    public void getByKey(String path, String key, FirebaseDataListener listener) {
    	log.info(">>IK Find key {} in path: {}", key, path);
        DatabaseReference childReference = databaseReference.child(path);
